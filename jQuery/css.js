var curCSS, iframe, iframeDoc,
    ...
    rrelNum = new RegExp("^([-+])=(" + core_pnum + ")", "i"), //用于相对值进行检测
    elemdisplay = {
        BODY: "block"
    },
...
jQuery.extend({
    ...
    style: function(elem, name, value, extra) {
        ...
        var ret, type, hooks,
            origName = jQuery.camelCase(name), //驼峰化
...
function getWidthOrHeight(elem, name, extra) {
    ...
    if (val <= 0 || val == null) {
        ...
        val = curCSS(elem, name);
        if (val < 0 || val == null) {
            val = elem.style[name]; //若值为空，则用默认样式
        }
        ...
        // 我们需要对浏览器可识别的值进行确认，并且保证能够回滚到默认值
...
jQuery.each(["height", "width"], function(i, name) {
    jQuery.cssHooks[name] = {
        get: function(elem, computed, extra) {
            if (computed) {
                //对于某些元素的尺寸信息，如果我们无法看到
                //但是又有必须的要显示的style对它有依赖
...
//在DOM加载后，利用hook处理bug,jQuery.support.reliableMarginRight在IE下为false，ff chrome 为true
...
//处理DOM元素是否占据布局空间即  offsetWidth／offsetHeight 是否为0。
//［这里需要注意visibility:hidden 或 opacity 为0时，元素依然占据布局空间］
    ...
    jQuery.cssHooks[prefix + suffix] = {
        expand: function(value) {
            var i,
                ...
                // assumes a single number ifnot a string
